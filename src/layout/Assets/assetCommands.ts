 ...
 *
 * Registration is idempotent; `AssetsPanel` installs on first mount.
 */
 ...
 *
 * A picker is a DOM `<input type=file>` that only opens from a user gesture
 * on the page that owns it, so the panel registers its openers while mounted
 * and the commands are enabled exactly then. Import goes to the PROJECT only
 * — the panel's toast offers "Add to composition" afterwards.
 */
  ...
  return [
    {
      ...
      icon: 'upload',
      enabled: () => importOpeners !== null,
      execute: () => { importOpeners?.files(); },
    },
    {
