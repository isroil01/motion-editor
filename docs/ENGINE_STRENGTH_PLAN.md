...
- Puppet position pins: "a small rigid disc per pin" — **tried and rejected 2026-09-10.** Constraining each pin's 1-ring rigidly in the pin's frame (the bend-pin core, one ring wide) measured worse on both counts that matter: the two-pin bent bar's smallest triangle fell from >40 % to 32 % of rest area (the disc eats the transition band on a coarse mesh), and placing a pin on an already-deformed character moved the artwork 17 px (its ring snapped to rest-relative offsets) where the placement contract allows 6. A disc would have to be sized in artwork px and anchored in deformed space at placement to be worth revisiting; the single-vertex handle with ARAP's rigid 1-ring fit stays.
